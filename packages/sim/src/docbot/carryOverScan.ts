...
export const CARRY_OVER_EXCUSED: Readonly<Record<string, CarryOverExcuse>> = {
  ...
  collectorBoughtThisTurn: { kind: 'no-combat-meaning', why: 'Rune of the Collector\'s per-turn minion-buy list (balance 9/23); buys are shop actions' },
  ...
  shoutFiresThisTurn: { kind: 'no-combat-meaning', why: 'Rune of Ancestral Roar\'s per-turn Shout-fire count: spent by the End-of-Turn lump BEFORE the fight is built; combat Shouts feed the cross-phase rune meters through QuestCombatMods.shoutMeters instead (balance 9/23)' },
