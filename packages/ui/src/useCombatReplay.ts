...
export function useCombatReplay(
  ...
  const questDelta = useMemo(() => {
    const d = {
      attack: 0, summonCombat: 0, slaughter: 0, deathrattle: 0, friendlyDeath: 0, rally: 0,
    ...
    for (const e of qe) {
      ...
      d[e.kind] += 1;
      if (e.kind !== 'deathrattle' && e.kind !== 'friendlyDeath' && e.kind !== 'rally') { // these carry no tribe breakdown
