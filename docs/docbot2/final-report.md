...
|---|---|---|---|
| **Contract in the committed registry** | 896 | 896 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 896 | 896 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 896 | 896 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 896 | 896 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
**Of those 896, 118 cover ARCHIVED content classes** — 117 quests and 1 henchman, both systems switched off by
...
| Contract verification depth | Count of 896 |
|---|---|
...
|---|---|---|
| `no-driver-for-shape` | 397 | **the largest real hole** — an applicable case with no executable driver yet |
...
- Graph: **1535** nodes / **4635** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **87140** candidate pairs against 400960 naive all-pairs (21.7%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 896/896 in the committed registry; `contractExtract.test.ts` fails on new content without one |
