...
import { hallRowsOf, hallRunKeyOf, parseRunKey, recordText, winRateText, type HallFightRecord } from './leaderboardData';
...
const facts = (over: Partial<{ heroId: string | null; rank: { divisionIndex: number; points: number } | null; record: { wins: number; losses: number; draws: number } | null; placement: number | null; board: { minions: unknown[]; runes?: string[] } | null }> = {}) =>
  ({ heroId: null, rank: null, record: null, placement: null, board: null, ...over });
...
describe('hallRowsOf', () => {
  ...
  it('joins the run\'s own career row by seed (guarded by hero): own record 12–3, its placement, the rank held, its board; falls back to a pool board', () => {
    const own = { minions: [{}, {}], runes: ['rune_x'] };
    const pool = new Map([['Kev|sable|8', { minions: [{}], runes: [] }]]);
    const history = new Map([
      [7, facts({ heroId: 'brackus', rank: { divisionIndex: 4, points: 12 }, record: { wins: 12, losses: 3, draws: 0 }, placement: 1, board: own })],
      [8, facts({ heroId: 'sable', record: { wins: 9, losses: 5, draws: 0 }, placement: 3, board: null })],
      [9, facts({ heroId: 'warden', record: { wins: 1, losses: 1, draws: 0 }, placement: 8, board: own })], // a different hero on that seed: never joined
    ]);
