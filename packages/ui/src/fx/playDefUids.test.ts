...
const UNIT_LESS: { file: string; id: string; why: string }[] = [
  ...
  { file: 'ancientsSmoke.ts', id: '<dynamic>', why: 'the Ancients smoke (arrival puff / settled haze): at an Ancient offer card, which is not a unit' },
  { file: 'ancientsSmoke.ts', id: 'ancient-slam', why: 'the Ancients reveal slam: lands under an Ancient offer card, not a unit' },
  { file: 'ancientsSmoke.ts', id: 'ancient-gate-smoke', why: 'the Ancients eruption smoke: bursts from the hero-power BUTTON (HUD chrome), no unit' },
  ...
  { file: 'useRuneArrivalFx.ts', id: 'rune-select-implosion', why: "the ARRIVAL implosion fires on a rune BADGE in the HUD tray as the lock-in ceremony hands the rune over — the subject is the rune, not a body on the board, and a badge has no unit uid (the same reason `runeTriggerFx` is here)" },
