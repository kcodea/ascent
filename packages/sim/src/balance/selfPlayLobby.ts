...
export function runSelfPlayLobby(
  ...
  while (!failure && !table.finished) {
    ...
    for (const seat of living) {
      ...
      const turn = playRecruitTurn(seat.run, seat.pilot, { seatId: seat.state.id, round, scoutedOpponent: foe?.run.lastCombat ?? null, ...selfPlayScout(seat, foe, ghost, living, round) }, rec, { maxActionsPerTurn, lobbyId, lineage: seat.lineage });
      seat.run = turn.run;
