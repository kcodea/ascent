...
/** Must match the `#bootsplash` opacity transition in index.html. */
const FADE_MS = 520;
...
export function Boot({ children }: { children: ReactNode }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    // Next frame, so the browser has painted the game underneath before the fade starts.
    const raf = requestAnimationFrame(() => el.classList.add('is-out'));
    const drop = (): void => el.remove();
    el.addEventListener('transitionend', drop, { once: true });
    const t = window.setTimeout(drop, FADE_MS + 250);
    return () => { cancelAnimationFrame(raf); window.clearTimeout(t); el.removeEventListener('transitionend', drop); };
  }, [ready]);
