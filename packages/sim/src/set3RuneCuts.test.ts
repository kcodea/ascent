...
describe('Set 3 rune cuts (owner 2026-09-24)', () => {
  ...
  it('the Set 3 static pool counts: 90 Basic / 84 Epic (owner Set 3 rune list 2026-09-25: 83 / 80, then rune batch 3 +7 / +4; was 119 / 108)', () => {
    const inS3 = (arr: typeof RUNES) => arr.filter((r) => !r.sets || r.sets.includes('set3'));
    expect(inS3(RUNES)).toHaveLength(90); // 83 → 90 on 2026-09-25 (Set 3 rune batch 3); 119 → 83 on 2026-09-25 (set3RuneList.test.ts pins the exact ids)
    expect(inS3(EPIC_RUNES)).toHaveLength(84); // 80 → 84 on 2026-09-25 (Set 3 rune batch 3); 108 → 80 on 2026-09-25
