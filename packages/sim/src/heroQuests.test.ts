/**
 * HERO QUESTS — Fi & Coran's reworked powers (owner spec 2026-08-21).
 *
 * Both heroes open the run on a two-option Discover drawn from their OWN quest list. Every hero quest shares
 * ONE objective — `journey`, +1 per minion played / spell cast / Shop upgrade — and differs only in count and
 * reward. The rules pinned here are the ones a refactor could silently drop:
...
/** Take option 0 of the opening offer so the shop unlocks and the quest is live. */
const start = (hero: string, seed = 1): RunState => reduce(createRun(seed, hero), { type: 'buyQuest', index: 0 });

/** The run's single active hero quest. */
const active = (s: RunState) => s.activeQuests![0]!;
const questOf = (s: RunState) => QUEST_INDEX[active(s).questId]!;
...
describe('the turn-1 offer', () => {
  it('is two options from that hero\'s own list, for both heroes, across many seeds', () => {
    for (const hero of ['fi', 'coran']) {
      const ids = heroIds(hero);
      for (let seed = 1; seed <= 30; seed++) {
        const offer = createRun(seed, hero).questOffer;
  ...
  it('taking one closes the modal and activates the quest at 0 progress', () => {
    const s = start('fi');
    expect(s.questOffer).toBeUndefined();
    expect(s.activeQuests).toHaveLength(1);
    expect(active(s).progress).toBe(0);
    expect(questOf(s).heroQuest).toBe('fi');
  });
});

describe('the journey counter', () => {
  ...
  it('buying, selling and rolling are NOT steps', () => {
    let s: RunState = { ...start('coran'), embers: 20 };
    const p0 = active(s).progress;
...
describe('rewards', () => {
  it('Spare Forge: completing hands over a random Basic Rune immediately', () => {
    const s = completeVia(start('fi'), 'hq_spare_forge');
  ...
  it('Gilded Shortcut: Gild needs only 2 copies', () => {
    const s = completeVia(start('coran'), 'hq_gilded_shortcut');
  ...
  it('Open Road: Tier 7 opens for the shop ladder, not just Discovers', () => {
    let s = completeVia(start('fi'), 'hq_open_road');
  ...
  it('Summit Passage: unlocks Tier 7 AND takes one free Shop step', () => {
    const s = completeVia({ ...start('coran'), tier: 3 }, 'hq_summit_passage');
    expect(s.tier7Access).toBe(true);
  ...
  it("Merchant's Road: shop minions cost 2 Gold", () => {
    const s = completeVia(start('coran'), 'hq_merchants_road');
