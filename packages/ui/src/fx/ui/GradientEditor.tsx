...
export function GradientEditor({ value, onChange, disabled }: { value: GradientStop[]; onChange: (s: GradientStop[]) => void; disabled?: boolean }) {
  ...
  return (
    <div className="fxwb-grad" aria-disabled={disabled || undefined}>
      <div className="fxwb-grad-bar" style={{ background: css }} onPointerDown={onBar}>
        {value.map((s, i) => (
          <span
            ...
            onPointerDown={(e) => { e.stopPropagation(); if (!disabled) setSel(i); }}
            onDoubleClick={(e) => { e.stopPropagation(); if (!disabled) { onChange(removeStop(value, i)); setSel(0); } }}
          />
        ))}
      </div>
      {value[sel] && (
