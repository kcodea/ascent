...
- A BUG FIX added its rule to the oracle in the same PR — an approved rule appended to its domain file
  `packages/rules/src/registry/approved/<domain>.ts` with the owner's words as evidence and the new
  regression test as its `enforcement.refs`, so Doc Bot catches the regression (owner ask 2026-09-22; recipe
  in CLAUDE.md). The report's rule counts are generated placeholders — do not edit `final-report.md`.
