...
- **New-minions batch — remaining (~10, wave 3+):** the heavy-subsystem tail of the ~27-card batch (waves 1–2
  shipped 16 cards — see devlog). Each needs a genuinely new mechanism: **baked auras** (Squirl Scout = Beasts
  ...
  **combat spell-cast** (Spell Drummer, Spark Capacitor, Hoardbreaker Drake — no combat "cast a spell" primitive
  ...
  destroy** (Graverobber); **adjacency consume** (Abyssal Feeder); and **Field Mechanic + Patch Job** — *Patch
  Job's "+3/+3 per attachment" needs a definition of "attachment"* (welded magnetics leave no clean count).
  Art still owed: Nimbus, Moe, Bounty Bot, Hoardbreaker Drake.
