...
const BALANCE_STAMPS = `${BALANCE_REV}, set_id, source`; // 2026-09-22 — the set + source stamps as columns
/** 2026-09-23: `player_key`, a server-side md5 of `user_id` generated by Postgres. The basis of every unique-
 *  player count; on a backend without it the rung below answers and the report counts display names, a proxy,
 *  and says so (`RunTelemetryFetch.playerKeyBasis`). */
const BALANCE_PLAYER = `${BALANCE_STAMPS}, player_key`;
...
export interface RunTelemetryFetch {
  ...
  cap: number;
  pageSize: number;
  /** Which key the rows carry for unique-player counts: `playerKey` when the select that answered read the
   *  `player_key` column, else `displayName` (an un-migrated backend; the report labels the proxy). */
...
export async function fetchRunTelemetry(opts: { cap?: number; pageSize?: number } = {}): Promise<RunTelemetryFetch> {
  ...
  const empty: RunTelemetryFetch = { rows: [], fetched: 0, truncated: false, cap, pageSize, playerKeyBasis: 'displayName' };
  const c = client();
  if (!c) return empty;
  try {
    ...
    const rows = raw.map((r) => {
      ...
      return {
        ...
        playerKey: typeof r.player_key === 'string' && r.player_key ? r.player_key : null,
