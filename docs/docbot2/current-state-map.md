...
| Retro catalog | `packages/sim/src/docbot/retroCatalog.ts` (data; measured verdicts per entry) + `npm run docbot:retro` (TypeScript runner, throwaway worktree) + `docbot-retro.yml` (weekly) | **done 2026-09-11** — the catch rate is derived by `npm run docbot:report` | §4.5, §17.3 |
...
| D-7 | **Retro catalog verdict drift** — verdicts in `docs/docbot-roadmap.md` prose, no verdict field, README says "0 of 7" | reinject.py catalog (retired 2026-09-11) | CLOSED: `retroCatalog.ts` carries a measured `verifiedBy` per entry, the PR gate refuses an unearned CAUGHT, and `docbot-retro.yml` re-measures weekly. |
