...
import { finalHoldMs, pummelReadMs, pummelRemainingMs } from './finalHold';
...
// `coerceLayer` drops every layer of every def (`getPrimitive` is empty), so `pummelReadMs` would fall back to
// the def's bare `duration` and the layer-span read below would be proven by nothing.
import '../fx/primitives';

/**
 * THE FINAL HOLD's pummel floor (owner ask 2026-09-21: a crossing on the last attack must still play its
...
const trigger = (source: string): CombatEvent => ({ type: 'pummelTrigger', source, side: 'player', marker: 'dealtDamageGoldNextTurn' } as CombatEvent);
...
describe('finalHoldMs', () => {
  ...
  it('a body fired twice in the last moment (synthetic) waits for the SECOND stacked detonation (one stride later, ÷ speed)', () => {
    const ev = [attack('hg', 'foe'), dmg('foe', 'hg'), trigger('hg'), trigger('hg'), death('foe')];
    const last = compileMoments(ev).at(-1);
    expect(finalHoldMs(last, ev, cardIds, { ...base, combatSpeed: 2 })).toBe(READ + PUMMEL_STACK_MS / 2);
  });
  ...
  it('a flash fired on an EARLIER beat is waited out through pendingPummelMs (Han Gover: the Ale + death beats trail the crossing)', () => {
    const ev = [attack('gv', 'foe'), dmg('foe', 'gv'), death('foe')]; // last beat: no crossing of its own
    const last = compileMoments(ev).at(-1);
    expect(finalHoldMs(last, ev, cardIds, { ...base, combatSpeed: 4, pendingPummelMs: 600 })).toBe(600);
    expect(finalHoldMs(last, ev, cardIds, { ...base, combatSpeed: 4, pendingPummelMs: 0 })).toBe(225);
  });
});

describe('pummelRemainingMs', () => {
  it('is what is left of the recorded flash at `now`, never negative, 0 with no fire', () => {
    expect(pummelRemainingMs(null, 5000)).toBe(0);
    expect(pummelRemainingMs({ at: 1000, readMs: 980 }, 1300)).toBe(680);
    expect(pummelRemainingMs({ at: 1000, readMs: 980 }, 1980)).toBe(0);
    expect(pummelRemainingMs({ at: 1000, readMs: 980 }, 9000)).toBe(0);
  });
});

describe('pummelReadMs — the real registry', () => {
  it('reads the owner’s pummel-trigger PAST its 900ms duration — the burst (at 100, life 880) ends at 980 — and well under the 3s an unmodelled sound tail would add', () => {
    const def = getDef('pummel-trigger')!;
    expect(def.layers.length).toBeGreaterThan(0); // the primitives ARE registered (else this test proves only `duration`)
    const read = pummelReadMs('k3_goldvein');
    ...
    expect(pummelReadMs('dw3_hangover')).toBe(read);
  });

  it('resolves the kind-level binding for an unknown card too (no card layer shadows it)', () => {
    expect(pummelReadMs(null)).toBe(pummelReadMs('k3_goldvein'));
  });
});
