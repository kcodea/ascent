...
export const KeywordSchema = z.enum(['T', 'DS', 'V', 'W', 'R', 'C', 'M', 'SC', 'CN', 'FD', 'IMM', 'ST', 'RL', 'SL', 'CR', 'EG', 'RB', 'RW']);
...
export const EffectFactoryIdSchema = z.enum([
  ...
  'onSpellCastOnThisRecastNamed', // Crash Course (2026-09-18; replaced the Adept's spread)
  ...
  'onBuyBuffStarform', 'goldSpentCreateStarformOrBuff',
  'buffThisShop',
  'equipmentBuffThisShop', 'equipmentCreateStarformThenBuffThisShop', 'endOfTurnCreateStarformThenBuff',
  ...
  'spellBuffLeftmost',
  'spellBuffRandomHand',
  'spellBuffRandomBoardAndHand',
  'spellBuffTargetAndRandomFriendly',
  'spellDestroyTargetGetSpell',
  'spellBuffLeftmostHandMinion',
  'spellBuffAllPerTribePlayed',
  'spellBuffTargetPerSpellsCast',
  'spellDiscoverMinion',
  'spellDiscoverShopSpell',
...
z.object({ kind: z.literal('runeAftermarket') }).strict(),
  ...
  z.object({ kind: z.literal('runeFiveBanners') }).strict(),
  z.object({ kind: z.literal('runeLassoing') }).strict(),
...
z.object({ kind: z.literal('runeThreshold'), meter: z.enum(['gold', 'spellCast', 'anySpell', 'spellCastNonAle', 'castRuby', 'cardsBought', 'cardsPlayed', 'playDragon', 'shout', 'consume', 'playSpirit']), per: z.number().int().positive(),
  ...
  grantGold: z.number().int().positive().optional(),
  improveRuby: z.object({ attack: z.number().int(), health: z.number().int() }).strict().optional(),
...
z.object({ kind: z.literal('consumeDoubleFirstEachTurn') }).strict(),
  ...
  z.object({ kind: z.literal('shoutEdgeBuff'), attack: z.number().int(), health: z.number().int(), tribe: TribeSchema.optional() }).strict(),
