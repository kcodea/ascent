...
/**
 * DEV-only "Cleave Slash FX" tuner — the hit-stop + red gash a Cleave attacker plays on connection
 * (`cleaveFxConfig` → the lunge's `hitStopMs` hold + `pixiFx.cleaveSlash`). Drag the sliders to tune by eye;
...
const LABELS: Partial<Record<keyof CleaveFxConfig, string>> = {
  ...
  returnDelayMs: 'RETURN delay ms',
  offsetX: 'offset X',
  offsetY: 'offset Y',
  scale: 'overall scale',
  spanPx: 'span px',
  slashCount: 'claw count',
  slashSpacing: 'claw spacing',
  slashWidth: 'claw width',
  slashTaper: 'claw taper',
  slashTilt: 'tilt°',
  slashJitter: 'tilt jitter°',
  slashBow: 'bow (sag)',
  slashStagger: 'claw stagger ms',
  sweepMs: 'rake ms',
  holdMs: 'hold ms',
  fadeMs: 'fade ms',
  retract: 'retract vs dissolve',
  coreAlpha: 'core alpha',
  glowWidth: 'glow width ×',
  glowAlpha: 'glow alpha',
  clawLen: 'claw length',
  clawWidth: 'claw width',
  clawRoot: 'claw root depth',
  clawBulge: 'claw bulge (0=root)',
  clawHook: 'claw hook',
  clawAlpha: 'claw alpha',
  clawFadeMs: 'claw fade ms',
  dripCount: 'drips / claw',
  dripSize: 'drip size',
  dripStretch: 'drip stretch',
  dripSpeed: 'drip speed',
  dripGravity: 'drip gravity',
  dripDrift: 'drip drift',
  dripLife: 'drip life ms',
  dripAlpha: 'drip alpha',
...
export function CleaveFxTuner() {
  ...
  const test = (): void => {
      // EXACTLY what a real cleave fires — same call, same dials, just centred on screen instead of on a card.
    // Keep it that way: a Test that took different arguments is precisely what made the panel lie.
    pixiFx.cleaveSlash(window.innerWidth / 2, window.innerHeight / 2);
