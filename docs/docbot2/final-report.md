...
|---|---|---|---|
| **Contract in the committed registry** | 921 | 921 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
...
| **Rulebook** | 133 rules (49 approved) | — | `enforcement.test.ts` — every approved rule names a backing lane file that must exist on disk |
...
**Of those 921, 118 cover ARCHIVED content classes** — 117 quests and 1 henchman, both systems switched off by
...
| Contract verification depth | Count of 921 |
|---|---|
| derived status **corroborated** (two independent sources agree — trace + text, or trace + params) | 382 |
...
|---|---|---|
| `no-driver-for-shape` | 421 | **the largest real hole** — an applicable case with no executable driver yet |
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 921/921 in the committed registry; `contractExtract.test.ts` fails on new content without one |
