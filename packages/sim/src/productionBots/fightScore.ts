...
export type FightPanel = 'pool' | 'procedural';
...
export function fightScore(v: BotVisibleState, panelSize = PANEL.length): FightResult {
  ...
  const real = poolPanel(v.setId, v.wave, n);
  const panel: FightPanel = real.length === n ? 'pool' : 'procedural';
  if (v.friendly.bodies.length === 0) return { winRate: 0, margin: -1, averageDamage: 1, fights: 0, panel };
  ...
  for (let i = 0; i < n; i++) {
    ...
    marginSum += damageMargin * 0.7 + survivorMargin * 0.3;
  }
  return {
