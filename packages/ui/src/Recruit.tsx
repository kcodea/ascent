...
export function tokenRefView( // exported for tokenRefView.test.ts (bug 86340900 regression)
  ...
  spellLive?: { a: number; h: number; ftb: number; ftbH: number; goldSpent: number; goldPouchValue?: number; tier?: number; growthBonus?: number; anySpellsThisTurn?: number },
  ...
  if (c.spell && spellLive) {
    return {
      ...
      text: spellDisplayText(c.id, spellLive.a, spellLive.ftb, spellLive.h, spellLive.goldSpent, spellLive.ftbH, spellLive.goldPouchValue ?? 0, { tier: spellLive.tier, rubyBonus, clueBonus, anySpellsThisTurn: spellLive.anySpellsThisTurn, growthBonus: spellLive.growthBonus }),
...
export function conjuredView(cardId: string, run: RunState): CardView | null { // exported for the cast preview layer (same live chain as the combat fly-ins)
  ...
  // The …Live readers, so a spell granted DURING combat flies in printing the value it would cast for right
  // now — the fight's spell power and escalation included (owner report 2026-09-22).
  const spellLive = {
    a: spellAttackBonusLive(run), h: spellHealthBonusLive(run),
    ftb: spellEscalationLive(run).attack, ftbH: spellEscalationLive(run).health,
    goldSpent: run.goldSpentThisTurn ?? 0, goldPouchValue: run.goldPouchValue, tier: run.tier,
    growthBonus: run.growthBonus, anySpellsThisTurn: anySpellsCastThisTurn(run),
  };
...
interface ShopViewOpts {
  ...
  tavernAtk?: number;
  tavernHp?: number;
  /** The per-source ledger behind the PERMANENT half (`run.tavernBuyBonusSources`) — the inspect breakdown
   *  prints one line per source ("Rune of Reinvestment", "Contract Butcher"…) instead of one "Tavern" line. */
  tavernSources?: Record<string, { atk: number; hp: number }>;
  ...
  alesThisTurn?: number;
  /** Held Equipment whose charge is still unspent this turn — Shredder's live End-of-Turn total. */
  ...
  /** Set 3 — spells of any kind cast this turn (Stellar Chorus's live total). */
  anySpellsThisTurn?: number;
...
export function liveOptsFromRun(run: RunState): ShopViewOpts { // exported for the Scene Builder's hover preview (same live-text chain as the shop)
  return {
    ...
    // The …Live readers fold the combat replay's display-only previews, so a Quil/Sporebat/Mammoth casting
    // Front to Back moves the HELD card's number as the cast happens rather than at settle (owner 2026-08-07),
    // and spell power gained mid-fight moves it too (owner 2026-09-22).
    spellBonus: spellAttackBonusLive(run), spellBonusH: spellHealthBonusLive(run),
    frontToBackBonus: spellEscalationLive(run).attack,
    growthBonus: run.growthBonus,
    frontToBackBonusH: spellEscalationLive(run).health,
    ...
    squirlScoutBuff: run.squirlScoutBuff, conductorBuff: run.conductorBuff, alesThisTurn: run.alesCastThisTurn, unusedEquipment: unusedEquipmentCount(run),
...
function offerLiveTextParams(golden: boolean, o: ShopViewOpts, cardId?: string): LiveTextParams {
  return {
    ...
    goldSpent: o.goldSpent ?? 0, goldSpentRun: o.goldSpentRun ?? 0, goldPouchValue: o.goldPouchValue ?? 0, playedThisTurn: o.playedThisTurn, squirlScoutBuff: o.squirlScoutBuff, conductorBuff: o.conductorBuff, alesThisTurn: o.alesThisTurn, unusedEquipment: o.unusedEquipment,
...
export function shopView(card: ShopCard, opts: ShopViewOpts = {}): CardView { // exported for shopSpellLiveText.test.ts
  ...
  // save) as "Shop Stats", so the lines still sum to what the offer promised.
  let restA = tavernAtk, restH = tavernHp;
  for (const [source, v] of Object.entries(opts.tavernSources ?? {})) {
    pushBuff(source, v.atk, v.hp);
    restA -= v.atk; restH -= v.hp;
  }
  pushBuff('Shop Stats', restA, restH);
  pushBuff('Tribe Bond',
  ...
  return {
    ...
    // `opts.minionCost` is the CHARGED price (`offerBuyPrice`: it already folds Moe's set `card.cost` and every
    // discount), so it wins over the raw `card.cost` — the coin must never show a price the buy would not take.
    cost: opts.freeFirstBuy ? 0 : (opts.minionCost ?? card.cost ?? CONFIG.minionCost), costChanged: opts.freeFirstBuy || card.cost !== undefined || (opts.minionCost !== undefined && opts.minionCost < CONFIG.minionCost),
...
export function Recruit() {
  ...
  // …Live: the combat replay's display-only spell-power preview folds in, so every spell surface prints the
  // value it would cast for AT THIS MOMENT of the fight rather than the pre-combat number (owner report
  // 2026-09-22). Outside a replay the preview is undefined and this is exactly `spellAttackBonus`.
  const spellBonus = spellAttackBonusLive(run);
  const spellBonusH = spellHealthBonusLive(run);
  // Front to Back's escalation, same treatment. Kept as two SCALARS (never the object) so the memos below can
  // depend on them without a fresh identity every render.
  const ftbLive = spellEscalationLive(run);
  ...
  // …and the same fold drives every printed SPELL number (owner report 2026-09-22: "front to backs text /
  // maybe all spells? not updating in real time from buffs in combat"). `simulate` keeps its own spell power
  ...
  const refViewsByUid = useMemo(() => {
    ...
    const add = (uid: string, cardId: string, owner?: { buffs?: { source: string; attack: number; health: number }[]; golden?: boolean }): void => {
      ...
      const spellLive = { a: spellBonus, h: spellBonusH, ftb: ftbBonus, ftbH: ftbBonusH, goldSpent: run.goldSpentThisTurn ?? 0, goldPouchValue: run.goldPouchValue, tier: run.tier, growthBonus: run.growthBonus, anySpellsThisTurn: anySpellsCastThisTurn(run) };
  ...
  const handViews = useMemo(
    ...
    [run.hand, run.tier, eotAnimStats, combatHandBuffs, spellBonus, spellBonusH, run.spellsThisTurn, run.deathrattlesTriggered, run.undeadAttackBonus, run.undeadHealthBonus, ftbBonus, run.wave, run.spellsCast, run.cardBuffs, run.fodderConsumedThisTurn, live, run.board, run.nextSpellExtraCasts],
  );
  ...
  const startDragSession = (drag: DragState, touch: boolean): void => {
    ...
    // A hand MINION plays (and the warband opens a slot to make room) once its centre clears this line. It
    // rides `getDragFeel().playLine` off the warband/hand rects; LOWER playLine = the warband reacts sooner
    // (opens the slot with less drag up). Falls back to the old "10% up into the play area" line only when the
    // hand is unmeasured.
  ...
  useEffect(() => {
    ...
    const tick = (): void => {
      ...
      if (!infiniteClockRef.current) observeTurnClock(next, run.wave); // the announcer's "Low on time" warning (10 s left, once per game)
