...
|---|---|---|---|
| **Contract in the committed registry** | 901 | 901 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
...
| **Rulebook** | 109 rules (33 approved) | — | `enforcement.test.ts` — every approved rule names a backing lane file that must exist on disk |
...
| Contract verification depth | Count of 901 |
|---|---|
...
|---|---|---|
| `no-driver-for-shape` | 402 | **the largest real hole** — an applicable case with no executable driver yet |
...
- Graph: **1552** nodes / **4695** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **93491** candidate pairs against 405450 naive all-pairs (23.1%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 901/901 in the committed registry; `contractExtract.test.ts` fails on new content without one |
...
| 14 | Mutation/sabotage tests demonstrate detection in every oracle family | **partial** | 26 of 52 docbot lanes carry an in-file sabotage proof, plus the retro harness's 14 reinjections; the remaining lanes are structural ratchets without one |
...
10. **Half the lanes lack a sabotage proof.** 26 of 52. An oracle that has never been shown to fail is an
