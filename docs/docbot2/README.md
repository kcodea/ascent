...
## The THREE RULES of a QA scenario (handoff §3 — the scenario format's own floor)

Recorded here because they used to live **only** in the `qaScenario.ts` docblock, where a reader who never
opened that file could not find them (WP A reconciliation item D-9i; closed by WP H):

1. **ONE ENGINE.** `runQaScenario` executes through the REAL `reduce` / `faceOmen` path — never a second
   implementation of card behaviour. A combat scenario pins its opponent into `servedBoards` and dispatches
   the real action, exactly as a live turn does.
2. **NO EXECUTABLE EXPECTATIONS.** Scenario files are pure JSON and the expectation vocabulary is closed and
   validated. A need the vocabulary cannot express becomes a `needs-ruling` entry, never embedded JS.
3. **DETERMINISTIC RESULTS.** The same scenario twice produces a byte-equivalent normalized result: no
   wall-clock, no `Math.random`, order-insensitive stringify, and the wave's opponent pinned before
   execution so the session-global pool can never leak into the outcome.
...
**Where the program stands against that gate:**
[`final-report.md`](final-report.md) — the WP H closing audit, generated by `npm run docbot:report` and
drift-gated on every PR. It carries the coverage matrix, the §21 checklist item by item
(**5 done · 11 partial · 0 not-started** at the time of writing), the retirement decisions for WP A's
D-1…D-10, and the honest blind-spot list. Read it before claiming any part of this program is finished.
