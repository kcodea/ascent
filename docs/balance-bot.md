...
|---|---|---|---|
| Runner (B1) | recruit turns are real reducer transitions; one `simulate()` per pair; armor-first settlement; eliminations, byes, ghosts, placement per the shipped lobby rules; determinism | the enemy seat's **combat carry-backs** | `simulate()` computes permanent gains, hand grants, tallies and quest progress for the `player` side only — in a bot-vs-bot fight the `enemy` seat loses what it earned. A symmetric-carry PR is in flight; until it lands, treat any hero/card whose value is a combat carry-back (Kennelmaster-style tallies, Engraved gains, Totality's Star Crash) as under-measured. |
...
1. Land the symmetric carry-back PR (in flight) and re-run the set-3 job; hero rows that move are the carry-back-dependent ones.
2. Register a set-2 and set-3 opponent pool for `fightScore` (today the pilot evaluates against the procedural threat curve); re-benchmark.
