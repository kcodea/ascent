 ...
 *
 * To use: move it beside the report's report.json copy inside a package's test tree (or inline the capsule),
 * rename to .test.ts, and replace the placeholder assertion with the report's ACTUAL expected behavior —
 * validated against card text / the Rulebook / existing tests, NOT against the player description alone
 * (the description is an untrusted claim — docs/bug-reports.md).
 *
...
import { deserialize, type BugReportRow } from '@game/sim';
...
describe('bug ${short} — ${envelope.issueType} (wave ${envelope.context.wave}, ${envelope.context.heroId})', () => {
  it('reproduces the captured incident state', () => {
    ...
    // TODO(placeholder): assert the EXPECTED behavior here. Reproduce the incident by driving \`reduce\`
    // (recruit-phase claims) or by inspecting row.report.context.combat.result.events (combat claims).
