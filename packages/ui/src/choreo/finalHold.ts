 ...
 *
 * ── the payload floor (owner ask 2026-09-21) ────────────────────────────────────────────────────────────
 ...
 * `payload-trigger` barely 100ms in. Nothing killed the play (a def owns its container; `clearParticles`
...
export interface FinalHoldInputs {
  ...
  pull: number;
  /** The wall-clock read of the def bound at `payloadTrigger` for a card, in ms — 0 when nothing is bound.
   *  Defaults to `payloadReadMs` (the real registry); injectable for the pure tests. */
  readMs?: (cardId: string | null) => number;
  /** Wall-clock ms still to run on a payload flash fired on an EARLIER beat (Han Gover's crossing is followed
   *  by its Ale's `toHand` beat and the death beat before the end) — `payloadRemainingMs` at the time the
   *  final hold is computed. 0 when none. */
  pendingPayloadMs?: number;
}
...
export function finalHoldMs(
  ...
  let hold = Math.max(inputs.finalHold / spd, inputs.pull > 0 ? inputs.pull + 100 : 0, inputs.pendingPayloadMs ?? 0);
  if (!last) return hold;
  const readMs = inputs.readMs ?? payloadReadMs;
  for (const f of payloadsFiredIn(last, events)) {
    const read = readMs(cardIds.get(f.uid) ?? null);
    if (read <= 0) continue; // nothing bound for this body → nothing on screen to wait for
    // A body that crossed N times plays N stacked detonations, the last starting (N-1) strides in (÷ speed,
    // matching the cue's own scheduling); wait for THAT one to finish.
    hold = Math.max(hold, read + ((f.count - 1) * PAYLOAD_STACK_MS) / spd);
  }
  return hold;
}

/** A payload flash the replay fired: when (wall clock) and how long it reads for (the def's visual end plus
 *  the stack stride for a multi-crossing body). Recorded by the hook's `onPayloadProc`. */
export interface PayloadFire {
  at: number;
  readMs: number;
}
...
export function payloadRemainingMs(fire: PayloadFire | null, now: number): number {
  if (!fire) return 0;
  return Math.max(0, fire.at + fire.readMs - now);
}

/**
 * The VISUAL end of one play of the def bound at `payloadTrigger` for `cardId`: the last non-sound layer to
 ...
 * a bound for retiring a stuck play, not a read. For the owner's `payload-trigger` (900ms; a 96-diamond burst
 * at 100ms living 880ms; a shockwave at 90ms) this is ~980ms. 0 when nothing is bound or the def is unknown.
 */
export function payloadReadMs(cardId: string | null): number {
  const binding = bindingFor(cardId, 'payloadTrigger');
  if (!binding) return 0;
