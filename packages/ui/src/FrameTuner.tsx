...
const cssText = (std: Vals, spell: Vals, stdTint: string, spellTint: string): string =>
  `.card.compact.stdframe.stdframe { ${declLine(STD_KNOBS, std)} --fovl: ${stdTint}; }\n` +
  `.card.compact.spellframe.spellframe { ${declLine(SPELL_KNOBS, spell)} --fovl: ${spellTint}; }`;
...
export function FrameTuner() {
  ...
  const style = useMemo(() => cssText(std, spell, stdTint, spellTint), [std, spell, stdTint, spellTint]);
  ...
  const copy = (): void => {
    ...
    const out =
      `.card.compact.stdframe { ${declLine(STD_KNOBS, std)} --fovl: ${stdTint}; }\n` +
      `.card.compact.spellframe { ${declLine(SPELL_KNOBS, spell)} --fovl: ${spellTint}; }`;
    void navigator.clipboard?.writeText(out);
    setCopied(true);
    window.setTimeout(() => setCopied(false), 1400);
  };
  const reset = (): void => { setStd(defaults(STD_KNOBS)); setSpell(defaults(SPELL_KNOBS)); setStdTint(DEF_TINT); setSpellTint(DEF_TINT); };
