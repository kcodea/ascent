...
> revision `476799594.7d7fe651.36adbc8b.qa1.b1.r2.m32`.
>
...
> for the machine-readable form. A double-brace token — the rule counts, `{{rules.total}}` and
> `{{rules.approved}}` — is a headline number the generator fills in when this file is read. It is never
> written as a literal, so a PR that adds a rule does not touch this document; `npm run docbot:report --
> --render` prints the filled-in copy.
>
...
|---|---|---|---|
| **Contract in the committed registry** | 1051 | 1051 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
...
contract's own case having been driven. That refusal is why the direct-execution number is 490 and not a
...
SKIP, never an execution, and 381 of the 486 driven contracts had at least one case actually observed.
...
- Graph: **{{interactions.graphNodes}}** nodes / **{{interactions.graphEdges}}** edges over the contract registry (content, effect-family, trigger-family,
...
measured verdicts on 2026-09-11: **17 CAUGHT, 1 MISSED**. The one miss is out of Doc Bot's remit (an owner
ruling that moved a design ceiling), so the **forward catch rate over the 17 in-scope bugs is 17/17**, and
over the trailing 30 days of report dates it is **4/4**. Three of the four September bugs were MISSED on the
morning run and CAUGHT the same afternoon as their lanes merged — Gifts by `entryPaths` and the free Rally
by `firePaths` (#1428), Rope Wrangler's doubled beats by `beatConservation` (#1426), which also moved that
class from out-of-scope into generic. The miss-driven loop working as designed. The ledger
(`retroInteractionMap.ts`) carries 17 rows established by a reinject run and 1 by class analysis; 10 of the
18 are multi-system bugs.
...
fails only on a REGRESSION (a ledger CAUGHT that now misses); a new MISSED never fails. So "17/17" means *17
...
- **{{rules.total}}** live rules: **{{rules.approved}}** hand-authored approved (`registry/approved/<domain>.ts`), the rest the owner's seeded
...
- Approved-but-unenforced: `R-PLAY-01`, `R-AURA-01` — pinned, shrink-only. The 2026-09-22 bug-fix sweep added
  ...
  sim and mounted-UI pins, as did `R-PRESENT-04` (the FX overlay's detach retires every live def play,
  2026-09-22), enforced by its headless detach / re-attach pin.
- Release blockers (approved rules the engine violates, visible every run, never silently green):
  ...
  PARTIAL by design while their fix PRs are in flight (`R-AVWIN-12` → #1618, `R-TEXT-04` → #1619); each
  names the open half and the pin to append on merge.
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 1051/1051 in the committed registry; `contractExtract.test.ts` fails on new content without one |
...
| 5 | Every active content object passes applicable generated scenarios or has visible failures | **partial** | {{contracts.withDirectExecution.of}} contracts had a case directly executed (36 before the 2026-09-11 family drivers), 0 metamorphic failures, 0 limit failures, one standing draft disagreement (`shaper`); 155 applicable cases still have no driver, each skip naming the scaler key / def field / trigger that keeps it out |
...
| 9 | Displayed text is checked against approved mechanics | **partial** | every object classified; 960 of 1045 parse fully and their amounts, counts, cadences, Ruby counts and trigger events are compared; 61 unresolved parses are refused, never called clean — and the contract side is still mostly unreviewed drafts |
...
2. **{{contracts.noDriverForShape}} contract shapes with no driver.** Still the single largest verification hole, though down from
...
| **Shrink the 69-object unresolved-parse queue** | Grammar by grammar, ratcheted grow-loudly under the 35% hard ceiling. The queue is ordered by content type in `npm run docbot:text` |
