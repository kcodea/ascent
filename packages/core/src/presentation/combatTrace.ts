...
export const COMBAT_TRACE_COVERAGE: Record<CombatEvent['type'], CombatTraceCoverage> = {
  ...
  payloadTrigger: { source: 'always', target: 'never', amount: 'never', note: 'a damage-meter crossing that PAID OUT (the future Payload keyword): source = the body whose meter crossed; marker (the meter factory) + side in detail; one event per credited crossing' },
};
