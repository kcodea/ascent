...
export type CohortRow = RunTelemetry & {
  id?: number | null;
  author?: string | null;
  /** The pseudonymous player key (`run_telemetry.player_key`, a server-side hash of the account id); null on a
   *  row with no account or on a backend that has not run the 2026-09-23 migration. */
 ...
 *  key: `run_telemetry.player_key`, a server-side md5 of `user_id` generated by Postgres, so one account is one
 *  key across renames and two accounts sharing a display name are two. The display name is the FALLBACK, a
 *  labelled proxy the panel and the export use ONLY when the backend has not run the 2026-09-23 migration (the
 *  fetch reports which basis it read; `playerKeyFor` picks the function). Neither key is ever written into a
 *  table or the export; only distinct counts and per-file aliases are. */
export type PlayerKeyOf = (row: CohortRow) => string | null;
/** Which key the report counts players by: the account key, or the display-name proxy on an un-migrated backend. */
export type PlayerKeyBasis = 'playerKey' | 'displayName';
export const accountKey: PlayerKeyOf = (row) => row.playerKey ?? null;
export const displayNameKey: PlayerKeyOf = (row) => row.author ?? null;
export const playerKeyFor = (basis: PlayerKeyBasis): PlayerKeyOf => (basis === 'playerKey' ? accountKey : displayNameKey);
...
export function runFacts(row: CohortRow, keyOf: PlayerKeyOf = accountKey): RunFacts {
 ...
 */
export function cardCohorts(rows: CohortRow[], keyOf: PlayerKeyOf = accountKey): { byCard: Map<string, CardCohorts>; coverage: CohortCoverage } {
...
export function mostProlificPlayer(rows: CohortRow[], keyOf: PlayerKeyOf = accountKey): { key: string; runs: number } | null {
...
export function tierDecisions(rows: CohortRow[], keyOf: PlayerKeyOf = accountKey): TierDecisionRow[] {
