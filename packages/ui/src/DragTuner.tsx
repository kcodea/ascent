...
const SPECS: Record<keyof DragFeel, [string, TunerUnit | undefined, string, string]> = {
  ...
  playLine:      ['Minion play line', '×', 'How far a MINION must be lifted from the hand before it plays and the warband opens a slot to make room. 0 = at the warband’s bottom edge; 1 = right at the hand’s top edge; negative sits it up in the field. Higher = the warband reacts SOONER (less drag up). Minions only.', 'Play & cast lines'],
  spellLine:     ['Spell cast line', '×', 'How far a SPELL must be lifted from the hand before it arms. 0 = up at the warband (a long drag); 1 = right at the hand’s top edge. Higher = less drag to cast. Spells only — minions unchanged.', 'Play & cast lines'],
