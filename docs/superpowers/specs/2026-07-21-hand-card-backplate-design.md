...
`apps/web/public/frames/cardplate.png` — source at
`C:\Users\micha\Desktop\Reference Art\cardplate.png`, **1023 × 1537**, aspect **0.6656**.

Loaded via the established pattern at `packages/ui/src/Card.tsx:21-38`: a module-const
`` `${import.meta.env.BASE_URL}frames/cardplate.png` `` plus a module-level `available` flag flipped
...
Plate geometry is locked to the source aspect: `height = width × 1.5024`.
...
- The variant art the owner first pasted (four corner diamonds) differs from `cardplate.png` (one
  bottom gem plus greek-key tabs). `cardplate.png` is the chosen source.
