...
describe('runAttackExchangeCues — the Rally hold scales with the proc count', () => {
  ...
  /**
   * UNCAPPED, by owner decision (2026-08-06: *"i want the full wind-up stretch, no matter how long it
   * takes"*), and this is the case that makes that a real choice rather than a formality.
   *
   * PR #897 rewrote Echohorn's proc loop from `mul(self)` to `(1 + echoExtras) * mul(self)`, so Sylus the
   * Reaper now multiplies it: a gilded Echohorn beside two gilded Sylus procs TEN times. Every proc is its
   * own pulse, so the swing holds for roughly three extra seconds. That is the intended behaviour — the
   * alternative (capping the pulses) was offered and declined — so it is pinned here rather than left to be
   * rediscovered as a hang.
   */
  it('scales linearly all the way up — ten procs holds nine strides longer, uncapped', () => {
    expect(withProcs(10) - withProcs(1)).toBeCloseTo((9 * RALLY_PROC_STRIDE_MS) / 1000, 3);
  });
});
