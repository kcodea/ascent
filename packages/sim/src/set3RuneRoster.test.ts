...
describe('the Set 3 static rune pool (handoff 2026-09-14)', () => {
  ...
  it('Set 1 and Set 2 pools keep their previous scoped runes — a carryover only ADDS set3', () => {
    ...
    expect([s1.filter((r) => !r.epic).length, s1.filter((r) => r.epic).length]).toEqual([102, 84]); // set 1: −1 Epic on 2026-09-25 (Seller's Market now Dwarf-gated; set 1 fields no Dwarves)
    expect([s2.filter((r) => !r.epic).length, s2.filter((r) => r.epic).length]).toEqual([130, 117]); // set 2 on 2026-09-25 (owner Set 3 rune list): +1 Basic / −1 Epic for Engraving Gems (Epic → Basic), +1 Epic for Lazarus (Undead gate removed; set 2 fields no Undead); set 2: −1 Epic on 2026-09-24 (Rune of the White Wolf archived with Moonhowl Mentor); set 2: Rune of Investment Basic → Epic (owner Ruby batch 2026-09-24); −1 Basic / −2 Epic on 2026-09-18 (tag pass: Undead/Spirit-body runes gate on tribes set 2 does not field)
  });
...
describe('a Set 3 Dwarf / Kobold run at the forge', () => {
  ...
  it('the forge pool matches the static pool sizes minus nothing structural (every set-3 rune reachable when all five tribes roll)', () => {
    ...
    expect(basic.length).toBe(65 + originals(false)); // → 65 on 2026-09-25 (Engraving Gems Epic → Basic) // → 99 (Investment → Epic); → 100 on 2026-09-24 (owner Set 3 rune cuts); // 112 → 110 on 2026-09-23 (Balance 9/23 archives); → 109 on 2026-09-24 (Hoardcalling gated `dragon` again)
    expect(epic.length).toBe(52 + originals(true)); // → 52 on 2026-09-25 (Engraving Gems Epic → Basic) // → 53 on 2026-09-25 (owner's Set 3 rune list); // → 79 (Investment → Epic); → 78 on 2026-09-24 (owner Set 3 rune cuts); // 90 → 87 on 2026-09-23 (Balance 9/23 archives)
  });
});
