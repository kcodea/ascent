...
describe('tierOf', () => {
  it('is tier 1 (plain silver) from 0, and 0 only below that', () => {
    expect(tierOf('attack', -1)).toBe(0);
    expect(tierOf('attack', 0)).toBe(1);
    expect(tierOf('attack', 49)).toBe(1);
  });
  it('steps up exactly AT each threshold (inclusive)', () => {
    expect(tierOf('attack', 50)).toBe(2);
    expect(tierOf('attack', 149)).toBe(2);
    expect(tierOf('attack', 150)).toBe(3);
    expect(tierOf('attack', 499)).toBe(3);
...
describe('crossedUp', () => {
  ...
  it('is null on a DOWN-cross (frame follows down, but no celebration)', () => {
    expect(crossedUp('attack', 600, 40)).toBeNull();
    expect(crossedUp('attack', 500, 499)).toBeNull();
  });
});
