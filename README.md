...
- **Shop consume FX** — eating a shop minion no longer plays the old ghost-Fred swirl. The eaten minion's ghost now **shakes**, **taffy-stretches** toward the eater, and is **pulled in** as it collapses, synced to a Pixi `consume-bands` source→target def. One GSAP timeline conducts it (the config `durationMs` is the clock), with a `🍖 Consume FX` dev tuner (shake / stretch / pull / duration + a show-stats toggle). It covers **every** shop eater for free — Bob Blart, Cinder Clerk, Godfodder, the Consume spell, the tavern auto-eat — because `playFodderEat` is the single choke point. (Combat consumption is out of scope; the real energy-bands look is an owner workshop follow-up.)
