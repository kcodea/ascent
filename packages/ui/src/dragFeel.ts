...
export interface DragFeel {
  ...
  collapseY: number;
  /** Spell cast/aim line — how far DOWN toward the hand a spell (targeted, untargeted, or Ruby) must be lifted
   *  before it arms, as a fraction of the gap from the warband's bottom to the hand's top: 0 = at the warband
  ...
  spellLine: number;
  /** Minion play line — the line a MINION must clear before it plays AND the warband opens a slot to make
   *  room. LOWER = the warband reacts SOONER (opens the slot with less drag up); higher = later. Owner-tuned by
   *  eye and can sit well below 0. Minions ONLY — the spell cast line is separate. Falls back to a fixed line
   *  when the hand is unmeasured. */
  playLine: number;
...
const DEFAULTS: DragFeel = {
  ...
  spellLine: 0,     // owner-tuned 2026-09-14
  playLine: -2,     // owner-tuned 2026-09-14: LOWER = the warband opens the slot sooner (reacts with less drag
                    // up). Owner set this well below the old slider floor for a very eager make-room.
...
export const DRAG_DESC: Record<keyof DragFeel, string> = {
  ...
  playLine: 'The line a MINION must clear before it plays and the warband opens a slot to make room. LOWER = the warband reacts SOONER (opens the slot with less drag up); higher = later. Can sit well below 0. Minions only.',
 ...
 */
export const DRAG_DEFAULTS_VERSION = 11;
