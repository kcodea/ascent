...
|---|---|---|---|
| **Contract in the committed registry** | 1015 | 1015 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 1015 | 1015 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 1015 | 1015 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 1015 | 1015 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
| Contract verification depth | Count of 1015 |
|---|---|
...
| derived status **extracted** (a draft nobody has corroborated yet) | 513 |
...
contract's own case having been driven. That refusal is why the direct-execution number is 486 and not a
...
**69 unresolved parses remain, a hard-ceilinged queue.** `textParse.test.ts` pins the count (the ratchet may
...
- Graph: **1761** nodes / **5634** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **141766** candidate pairs against 514605 naive all-pairs (27.5%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 1015/1015 in the committed registry; `contractExtract.test.ts` fails on new content without one |
...
| 9 | Displayed text is checked against approved mechanics | **partial** | every object classified; 946 of 1015 parse fully and their amounts, counts, cadences, Ruby counts and trigger events are compared; 69 unresolved parses are refused, never called clean — and the contract side is still mostly unreviewed drafts |
...
1. **69 unresolved parses, and draft contracts on the other side.** The parser now reads 93% of printed text,
