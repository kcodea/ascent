...
export function PalettePicker({ value, onChange, disabled }: { value: Quad; onChange: (v: Quad) => void; disabled?: boolean }) {
  const [sel, setSel] = useState(2);
  const setStop = (c: number) => onChange(value.map((v, i) => (i === sel ? c : v)) as Quad);
  return (
    <div className="fxwb-pal" aria-disabled={disabled || undefined}>
      <div className="fxwb-pal-stops">
        {value.map((c, i) => (
          <button key={i} className={`fxwb-pal-stop${i === sel ? ' sel' : ''}`} onClick={() => { if (!disabled) setSel(i); }}>
            <span className="sw" style={{ background: numToHex(c) }} />
            <span className="nm">{PALETTE_STOP_LABELS[i]}</span>
          </button>
        ))}
      </div>
      <ColorPickerHSB value={value[sel]} onChange={setStop} disabled={disabled} />
