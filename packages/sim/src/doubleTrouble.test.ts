...
/**
 * TROUBLE (set 3) — "When a Ruby is cast on another minion in combat, cast a Ruby on this."
 *
 * Renamed from Double Trouble (owner, 2026-08-31). The card ID is unchanged (`k3_doubletrouble`), which is
 * what saves, replays and art all key on — and why this file keeps its name too.
 *
...
describe('Trouble', () => {
  ...
  it('a Ruby on another minion casts one on Trouble', () => {
    ...
    expect(rubyHealthOn(r, 'm0'), 'Trouble took one too').toBeGreaterThan(0);
  });
  ...
  it('a Ruby cast on DOUBLE TROUBLE ITSELF does not trigger it', () => {
    // "Another minion" — the separation. Board order puts the rattler beside Trouble, so its Ruby
  ...
  it('two Troubles do not feed each other', () => {
