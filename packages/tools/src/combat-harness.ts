...
function describe(ev: CombatEvent, names: Map<string, string>): string {
  ...
  switch (ev.type) {
    ...
    case 'spellcast':
      return `  ✨ ${ev.side} casts a spell (#${ev.count})`;
    case 'proccrit':
    ...
    case 'wardDowngrade':
      return `   ◇ ${n(ev.target)}'s Resilient Ward absorbs it (a plain Ward remains)`;
    case 'shieldUp':
...
const METER_SEED = 0x9a71d;
const m1 = simulate(meterPlayer, meterEnemy, makeRng(METER_SEED), CARD_INDEX);
...
console.log(
  ...
  } result  ${meterDeterministic ? '✓' : '✗ — BUG'}`,
);

if (!deterministic || !meterDeterministic) process.exit(1);
