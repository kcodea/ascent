...
const SPECS = {
  ...
  squashX: {
    ...
    help: 'Horizontal scale of the stream — 1 is untouched, lower squeezes it narrow, higher stretches it wide. Scales BOTH the spawn area and the speed particles fly at, so it holds its shape at any speed.',
  },
  squashY: {
    kind: 'slider', label: 'Squash Y', group: 'Physics', min: 0.2, max: 3, step: 0.01, default: 1, essential: true,
    help: 'Vertical scale of the stream — 1 is untouched, lower flattens it into an oval, higher makes it taller than it is wide. Pair this with Squash X: equal values resize, unequal values reshape.',
  },
  offsetX: {
    ...
    help: 'Shifts the stream sideways from its anchor, in px. Placement only — it moves where the effect sits without changing its shape or motion.',
  },
  offsetY: {
    kind: 'slider', label: 'Offset Y', group: 'Physics', min: -200, max: 200, step: 1, default: 0, axis: 'scale', essential: true,
    help: 'Shifts the stream up or down from its anchor, in px. Negative is up. Placement only — it moves where the effect sits without changing its shape or motion.',
  },
  inheritVel: {
...
class EmitterInstance implements FxInstance<EmitterParams> {
  ...
  constructor(ctx: FxContext, params: EmitterParams) {
    ...
    // frozen per-mote draw order is untouched. `fieldPhase` 0 → offset 0 → clockSec starts at 0 as before.
