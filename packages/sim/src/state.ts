...
export interface BoardCard {
  ...
  chosenOption?: number;
  /** Choose One: this instance resolved EVERY branch when it was played (a (Both) play: a charge, a rune, a
   *  golden `chooseBothWhenGolden`). Recorded so Rune of Sold Choices can repeat what it did (owner 2026-09-25);
   *  `chosenOption` stays unset on such a body, which keeps its combined text. */
  chosenBoth?: boolean;
...
export interface RunState {
  ...
  /** Rune of Gemmed Decisions: after you play a Choose One card, get a Ruby (per copy held). */
  runeGemmedDecisions?: boolean;
  /** Rune of the Red Storm: every Veinstorm cast also casts a Ruby on 2 random friendly Kobolds (per copy held). */
  runeRedStorm?: boolean;
  /** Rune of Choices: a Choose-Both charge per copy held is armed at every turn setup. */
  runeChoices?: boolean;
  /** Rune of Storming Veins: a Veinstorm cast FROM HAND casts this many additional times (2 per copy held). */
  runeStormingVeins?: number;
  /** Rune of Sold Choices: selling a board Choose One minion repeats the option it chose when played. */
  runeSoldChoices?: boolean;
  /** Rune of Combatative Rubies' running meter: friendly attacks since the last payout (0-2), across fights. */
  runeCombatativeTick?: number;
  /** Rune of Body Counting's running meter: friendly deaths since the last payout (0-7), across fights and Shop. */
