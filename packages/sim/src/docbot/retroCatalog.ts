...
export const RETRO_CATALOG: readonly RetroCatalogEntry[] = [
  ...
  // ── wave 3 (2026-09-11): Bug Board round 2 (PR #1374) — the four engine fixes, verdicts as measured ──
  {
    ...
    patch: [{ file: REC,
      find: "() => captureBuffFx(ctx.state, undefined, 'spell', () => fn(ctx, target as BoardCard, params, { minion: target as BoardCard, target: target as BoardCard }), cardCastSpellId),",
      replace: "() => captureBuffFx(ctx.state, undefined, 'spell', () => fn(ctx, target as BoardCard, params, { minion: target as BoardCard }), cardCastSpellId), // REINJECT: cast path drops the target (Bug Board 9852e16f)" }],
