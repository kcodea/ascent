...
describe('set 3 scaffold', () => {
  it('holds only the Equip minions, and the shared neutral spell toolkit', () => {
    ...
    // The Celestial rework has not landed; what is here is the EQUIPMENT work — Alchemist Frank (the handoff's
    // reference card) and Titan Sculptor, the second Equipment, which is what puts two in play at once.
    expect(p.buyable.map((c) => c.id)).toEqual(['e3_frank', 'e3_sculptor']);
