...
describe('the every-2-turns runes — one shared `everyTurns` cadence, not three flags', () => {
  ...
  it.each(CADENCE)('%s pays on the SECOND turn, not the first', (id, cardId) => {
    ...
    // Repeat every 2 turns"), so the cadence is measured as copies GAINED from the purchase count, not from zero.
    const count = (): number => held(s).filter((c) => c === cardId).length;
    const start = count();
    expect(start, `${id}: immediate copy on purchase`).toBe(id === 'rune_muckbroker' ? 1 : 0);
    const turn = (): void => {
      s = reduce(s, { type: 'faceOmen' }) as RunState;
      s = reduce(s, { type: 'resolveCombat' }) as RunState;
    };
