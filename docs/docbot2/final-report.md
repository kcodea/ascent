...
|---|---|---|---|
| **Contract in the committed registry** | 1047 | 1047 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 1047 | 1047 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 1047 | 1047 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 1047 | 1047 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
| Contract verification depth | Count of 1047 |
|---|---|
| derived status **corroborated** (two independent sources agree — trace + text, or trace + params) | 505 |
...
- Graph: **1820** nodes / **5702** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **142606** candidate pairs against 547581 naive all-pairs (26%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 1047/1047 in the committed registry; `contractExtract.test.ts` fails on new content without one |
