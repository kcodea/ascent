...
export function exportReadme(): BalanceExport['readme'] {
  return {
    ...
    aggregates: {
      ...
      demand: 'Per card AND content revision, from the derived streams. cardId = the card; rev = that card\'s content revision when it was offered (rows are never pooled across revisions); copiesOffered and copiesBought = individual copies offered in a tavern and bought; copyConversion = copiesBought over copiesOffered; shopsWithCard and shopsConverted = tavern turns showing the card and those that led to a buy; shopConversion = their ratio; runsOffered and runsAcquired = runs offered it and runs that got it by any source; runAcquisitionRate = their ratio; copiesPerShopAppearance = mean copies bought per shop appearance; bySource = acquisitions by source (shop, discover, quest, rune, heroPower, henchman, generated); acquisitions = the total; playRate = share of acquisitions ever played to the board; finalBoardRate = share still on the final board; avgAcquiredWave = mean wave of acquisition. Rates are fractions 0 to 1, null without a denominator.',
      economy: 'Per wave, the Gold flow of the runs that reached it. wave = the wave; runs = runs that reached the wave (the divisor); avg = the mean Gold per such run, one number per category: income (Gold that came in), minion, spell, ruby, refresh, upgrade, heroPower, rune and henchman (spends, as positive outlay), sell (Gold recovered by selling, an inflow) and other. A category no run used reads 0.',
