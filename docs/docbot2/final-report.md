...
|---|---|---|---|
| **Contract in the committed registry** | 931 | 931 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 931 | 931 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 931 | 931 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 931 | 931 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
| Contract verification depth | Count of 931 |
|---|---|
| derived status **corroborated** (two independent sources agree — trace + text, or trace + params) | 390 |
...
|---|---|---|
| `no-driver-for-shape` | 431 | **the largest real hole** — an applicable case with no executable driver yet |
...
**541 unresolved parses is the honest headline of this section.** They are classified, queued, ratcheted
...
- Graph: **1593** nodes / **4961** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **100988** candidate pairs against 407253 naive all-pairs (21.9%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 931/931 in the committed registry; `contractExtract.test.ts` fails on new content without one |
