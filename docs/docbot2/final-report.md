...
|---|---|---|---|
| **Contract in the committed registry** | 1048 | 1048 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 1048 | 1048 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 1048 | 1048 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 1048 | 1048 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
| Contract verification depth | Count of 1048 |
|---|---|
| derived status **corroborated** (two independent sources agree — trace + text, or trace + params) | 506 |
...
| **with at least one case a driver actually EXECUTED this sweep** | 488 |
...
contract's own case having been driven. That refusal is why the direct-execution number is 488 and not a
...
- Graph: **1822** nodes / **5708** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **142974** candidate pairs against 548628 naive all-pairs (26%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 1048/1048 in the committed registry; `contractExtract.test.ts` fails on new content without one |
...
| 9 | Displayed text is checked against approved mechanics | **partial** | every object classified; 958 of 1048 parse fully and their amounts, counts, cadences, Ruby counts and trigger events are compared; 61 unresolved parses are refused, never called clean — and the contract side is still mostly unreviewed drafts |
