...
function reduceCore(state: RunState, action: Action): RunState {
  ...
  switch (action.type) {
    ...
    case 'sell': {
      ...
      {
        const soldEq = sold && s.runeQuickRelease ? equipmentOf(CARD_INDEX[sold.cardId]) : undefined;
        if (soldEq) s.quickReleaseArmed = { excludeEquipmentId: soldEq.id };
      }
