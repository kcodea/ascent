...
- **An FX workbench.** Adding a new visual effect no longer costs a hand-written tuner panel (there were 34,
  ...
  draggable **value-over-life curves** (an effect's magnitude can animate across its own lifetime — first
  wired as particle size-over-life), a **Fire** button that plays an effect once to true completion decoupled
  from the loop, and staged **scenarios** (two units, pinned-to-cursor, click-to-place, ping-pong bounce,
  stationary) so an effect can be tuned without playing to the moment it fires. Four primitives so far — a
  posterized cel-band **trail**, a radial **burst**, an expanding **shockwave**, and a continuous **emitter**
  — with more depth and effect-stacking queued. The whole workbench tree-shakes out of the production build.
  See the [devlog](docs/devlog.md).
