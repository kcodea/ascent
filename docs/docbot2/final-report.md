...
| **Rulebook** | 169 rules (83 approved) | — | `enforcement.test.ts` — every approved rule names a backing lane file that must exist on disk |
...
- **169** live rules: **83** hand-authored approved (`registry/approved.ts`), the rest the owner's seeded
  queues folded through `decisions.json` — **146** read approved, 12 revised, **2** still needs-ruling.
...
- Approved-but-unenforced: `R-PLAY-01`, `R-AURA-01` — pinned, shrink-only. The 2026-09-22 bug-fix sweep added
  ...
  `R-REPEAT-01` (the LUMP / REPEAT text patterns: Mother Moss + Kringle repeat per tick, 2026-09-22) joined
  the same day, enforced by `repeatPerTick.test.ts` + `repeatPerTickBeats.test.ts`.
