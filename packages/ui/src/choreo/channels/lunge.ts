...
export interface LungeCtx {
  ...
  hitStopMs?: number;
  /** ms the attacker holds at the contact pose AFTER the impact fires, before settling home — so a
   *  Cleave's slash gets to play into a held frame instead of chasing a departing card. Rides the
   *  speed-scaled timeline. 0 (default) = settle immediately, the normal swing. */
...
export function playLunge(ctx: LungeCtx): ReturnType<typeof gsap.timeline> {
  ...
  // …and then hold again AFTER the impact fires, so a Cleave's slash rakes across a still board before the
  // attacker pulls home. Same empty-tween mechanism; 0 for an ordinary swing, which settles immediately.
  const returnS = Math.max(0, returnDelayMs) / 1000;
  if (returnS > 0) tl.to({}, { duration: returnS });
