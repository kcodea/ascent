...
import { OPPONENT_POOL_DATA } from '../../sim/src/opponentPool.data';
...
const poolForSet = waves.flatMap((w) => curateWave(byWave.get(w)!, CAP_PER_WAVE).sort((a, b) => a.power - b.power));
...
for (const s of poolForSet) bands[ratingBand(s.rating ?? 0)]++;
const bandReport = bands.map((n, i) => `b${i}:${n}`).join(' ');
const perWave = waves.map((w) => {
  const bs = poolForSet.filter((s) => s.wave === w).map((s) => ratingBand(s.rating ?? 0));
  return `w${w}:${bs.length}[b${Math.min(...bs)}–b${Math.max(...bs)}]`;
});

// PRESERVE the other sets' boards. The bake writes ONE committed file, so re-baking set 2 would otherwise
// wipe set 1's pool (and vice versa) — the boards are set-stamped and filtered at pick time, so they coexist
// happily in one array; only the set being baked is replaced.
const kept = OPPONENT_POOL_DATA.filter((b) => (b.setId ?? 'set1') !== SET_ID);
if (kept.length > 0) console.log(`  preserved ${kept.length} boards from other sets`);
const pool = [...kept, ...poolForSet];
 ...
 * ${pool.length} boards (${poolSynth} synthetic + ${poolReal} imported) across waves ${waves[0]}–${waves[waves.length - 1]}.
 * Set '${SET_ID}' baked under ${PATCH}; boards from other sets are preserved across bakes (each board is
 * setId-stamped and only served to a run playing that set — see docs/card-sets.md).
