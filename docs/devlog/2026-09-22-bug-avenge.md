...
- **The sim was right.** `placeSummon` (`packages/core/src/combat/simulate.ts`) stamps
  `avengeBaseline = deaths[side]` on every mid-combat summon (the 2026-08-24 fix, rule R-AVWIN-01 "Late entry
  starts at zero"), and `avengeCountFor` subtracts it. Bullseye's Echo goes through `ctx.summon` →
...
Patch note: Systems (readout only, no rule change).
