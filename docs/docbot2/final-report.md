...
|---|---|---|---|
| **Contract in the committed registry** | 996 | 996 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 996 | 996 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 996 | 996 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 996 | 996 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
| Contract verification depth | Count of 996 |
|---|---|
...
**61 unresolved parses remain, a hard-ceilinged queue.** `textParse.test.ts` pins the count (the ratchet may
...
- Graph: **1728** nodes / **5604** edges over the contract registry (content, effect-family, trigger-family,
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 996/996 in the committed registry; `contractExtract.test.ts` fails on new content without one |
...
| 9 | Displayed text is checked against approved mechanics | **partial** | every object classified; 929 of 996 parse fully and their amounts, counts, cadences, Ruby counts and trigger events are compared; 61 unresolved parses are refused, never called clean — and the contract side is still mostly unreviewed drafts |
...
1. **61 unresolved parses, and draft contracts on the other side.** The parser now reads 94% of printed text,
...
| **Shrink the 67-object unresolved-parse queue** | Grammar by grammar, ratcheted grow-loudly under the 35% hard ceiling. The queue is ordered by content type in `npm run docbot:text` |
