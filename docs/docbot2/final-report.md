...
|---|---|---|---|
| **Contract in the committed registry** | 895 | 895 (100%) | `contractExtract.test.ts` — new content FAILS the PR gate until `npm run contracts:extract` is committed |
| **Text classification bucket** | 895 | 895 (100%) | `textParse.test.ts` — every object lands in exactly one of four buckets; an unresolved parse is a queue entry, never a pass |
| **Contract-oracle applicability** | 895 | 895 (100%) | `contractOracle.test.ts` — every contract is planned into §10.1 case templates; every unexecuted applicable case carries a typed skip reason |
| **Interaction-graph membership** | 895 | 895 (100%) | `interactionGraph.test.ts` — every contract is a graph node; unmapped triggers are reported as a visible to-do (29 today) |
...
895 contracted objects in total (901 until 2026-08-28, when the owner archived the 16 Celestials — "leaving set 3 empty of minions now" — and every count in this report moved with them; +2 again the same day for the two Equipment reference cards).  Of those contracts, 13 are hand-authored curated ones and the rest are
extractor drafts, visibly `reviewStatus: 'extracted'` (§4.2 — a machine guess is never silently intent).

**Of those 895, 118 cover ARCHIVED content classes** — 117 quests and 1 henchman, both systems switched off by
...
| Contract verification depth | Count of 895 |
|---|---|
...
|---|---|---|
| `no-driver-for-shape` | 396 | **the largest real hole** — an applicable case with no executable driver yet |
...
| Bucket | Objects |
|---|---|
| parsed-equivalent | 357 |
| verified-mismatch | 9 |
| approved-exception | 0 |
| **unresolved-parse** | 518 |
...
**518 unresolved parses is the honest headline of this section.** They are classified, queued, ratcheted
...
- Graph: **1532** nodes / **4624** edges over the contract registry (content, effect-family, trigger-family,
  channel, keyword, multiplier, copy-mode, counter, zone, phase-boundary nodes).
- Applicability: **86813** candidate pairs against 400065 naive all-pairs (21.7%) — the producer → channel →
...
|---|---|---|---|
| 1 | Every active card, rune, hero power, keyword, token and supported system has a semantic contract | **done** | 895/895 in the committed registry; `contractExtract.test.ts` fails on new content without one |
