...
/**
 * DEV-only tuner for the replay ROUND RAIL (owner ask 2026-08-19). Applies live via `--rrl-*` vars; shipping a
 * look means pasting the JSON into replayRailConfig DEFAULTS *and* mirroring it into the styles.css fallbacks.
 * Only visible in action during a replay — start one (end screen → Rewatch) before dialing. `x`/`y` are the
 * rail's HOME; a viewer's own drag offset (persisted separately) rides on top of them.
 */
const SPECS: Record<keyof ReplayRailConfig, [string, TunerUnit | undefined, string, string]> = {
  x:          ['Horizontal', 'px', 'How far from the viewport’s left edge the rail’s home sits.', 'Rail'],
  y:          ['Vertical', 'px', 'Vertical nudge off dead-center. Negative lifts it.', 'Rail'],
  s:          ['Size', '×', 'Overall scale — every column together.', 'Rail'],
  colW:       ['Column width', 'px', 'Width of each data column (Gold / Acts / Tier / Power / Win %).', 'Columns'],
  collapsedW: ['Collapsed width', 'px', 'Width of the slim handle the rail collapses to.', 'Collapsed'],
};

/** Declaration order IS render order, and controls sharing a group render together under its heading. */
const ORDER: (keyof ReplayRailConfig)[] = ['x', 'y', 's', 'colW', 'collapsedW'];
