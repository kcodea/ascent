...
export interface EotStepFx {
  ...
  /** Host uids that gained an Attachment on this beat (Combinator, Cling Drones, Money Bots) — the UI fires
   *  the weld pulse on them as the beat plays, since the real weld's stamp lands after the phase flips. */
  welds: string[];
}
...
export function projectEndOfTurnSteps(state: RunState): {
  ...
  const beat = (source: BoardCard | undefined, run: () => void): void => {
    ...
    // Welds this beat produced — diffed by host `attachments` count, so EVERY auto-weld path is caught
