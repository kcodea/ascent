  ...
  switch (action.type) {
    ...
    case 'play': {
      ...
      if (playedDef?.target === 'friendly' && !playedDef.chooseOne?.length) {
        // R-TARGET-03 (owner 2026-09-18, global): an aimed Shout NEVER targets its own body — a board holding
        // ONLY this minion has no legal pick, so don't prompt; the Battlecry simply doesn't fire and it plays
        // as a plain body. (Was Graverobber's `targetNotSelf` opt-in; now the rule for every aimed Shout.)
