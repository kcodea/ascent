 ...
 *   npm run balance:run     -- --manifest <experiment.json> [--out <jobId>]
...
export async function main(argv: readonly string[] = process.argv.slice(2)): Promise<void> {
  ...
  switch (cmd) {
    ...
    default:
      console.log('usage: balance <report|compare|synth|list|run|matrix|findings> [--flags]  (see packages/tools/src/balance/cli.ts)');
  }
}
