...
- **Fel Spikes' Echo throws a spike volley** — its Deathrattle now fires the `fel-spike` effect from the dying body to every minion it hits, all at once. (Combat damage events now record who dealt them, so a source→target FX can launch from an AoE's caster.)
