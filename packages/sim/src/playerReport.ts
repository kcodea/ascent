...
export function exportReadme(): BalanceExport['readme'] {
  return {
    ...
    meta: {
      ...
      quality: 'Data-integrity counts over the exported rows: rows; placementMissing (no placement) and placementMalformed (cleared); duplicateIds; withDerived, diverged (partial payloads, left out of every derived read) and stackedStreams (payloads whose streams carried earlier runs of the same browser session in front of their own, read by their last segment only); replayDisagree = rows whose replay-derived tierByWave does not span the live finalWave (the flat tier table and shop curve are unreliable for them); heroOfferMissing = rows with no recorded hero trio; revisionMissing = rows with no content revision stamp.',
    ...
    runs: {
      ...
      author: 'The player\'s display name at the time. Several runs share an author when one player played several runs. A display name, never an account id; the unique-player counts in the tables count these names.',
