...
import { HEROES, playableHeroes, runSelfPlayLobby, SETS, type BalanceRecorder, type ExperimentIdentity, type ExperimentManifest, type LobbyRecord, type SeatPilot } from './deps';
...
export async function runnerFor(mode: ExperimentManifest['mode']): Promise<LobbyRunner> {
  if (mode === 'selfPlayLobby') return runSelfPlayLobby;
  if (mode === 'pinnedLobby') {
    const spec = '@game/sim/balance/pinnedLobby';
    let mod: { runPinnedLobby?: unknown };
    try { mod = (await import(/* @vite-ignore */ spec)) as { runPinnedLobby?: unknown }; } catch { throw new Error('balance:matrix — mode "pinnedLobby" needs the pinned-lobby runner (`@game/sim/balance/pinnedLobby`, branch feat/balance-pinned), which is not on this branch'); }
    if (typeof mod.runPinnedLobby !== 'function') throw new Error('balance:matrix — `@game/sim/balance/pinnedLobby` does not export runPinnedLobby');
    // runPinnedLobby(manifest, seed, pilot, recorder, identity): one pilot (seat 0) — adapt the factory shape.
    const run = mod.runPinnedLobby as (manifest: ExperimentManifest, seed: number, pilot: SeatPilot, recorder: BalanceRecorder, identity: ExperimentIdentity) => LobbyRecord;
    return (manifest, seed, pilotFor, recorder, identity) => run(manifest, seed, pilotFor(0), recorder, identity);
  }
